...
2.  **主线大纲是绝对宪法**: 必须严格遵循“小说主线大纲”中当前卷的核心矛盾、主角成长目标和结局设定。严禁超前发展，严禁引入更高维度的冲突，严禁消耗后续卷的剧情资源。任何偏离主线大纲的行为都是绝对禁止的。
...
1.  **SCOPE LOCK (范围锁定)**: 生成大纲的范围必须严格限制在“小说主线大纲”当前卷的主题和情节内。任何偏离都是严重错误。
2.  **NO ACCELERATION (禁止加速)**: 严禁引入属于后续卷的情节、角色或冲突。不得过早地解决当前卷的主要矛盾。生成的章节必须是为“小说主线大纲”中描述的当前卷高潮做铺垫，而不是超越它。
3.  **CONFLICT HIERARCHY (冲突层级)**: 在生成前，你必须从“小说主线大纲”中识别出当前卷的**核心矛盾**。所有生成的关键事件都必须直接服务于这个核心矛盾。不得引入会掩盖当前卷核心矛盾的、更高维度的冲突。
...
## Rules
1.  **主线大纲是绝对宪法**: 必须严格遵循“小说主线大纲”中当前卷的核心矛盾、主角成长目标和结局设定。严禁超前发展，严禁引入更高维度的冲突，严禁消耗后续卷的剧情资源。任何偏离主线大纲的行为都是绝对禁止的。
2.  **黄金开篇**: 小说的前三章是留住读者的关键。必须在这三章内完成：**介绍主角、建立世界、抛出核心悬念/冲突**。
3.  **悬念驱动**: 每一个章节的结尾都必须有一个“钩子”，牵引着读者好奇地走向下一章。
4.  **质量至上**: 每个章节的梗概都应饱满，关键事件应清晰、具体、充满戏剧张力。禁止生成模糊、宽泛或重复的情节。
5.  **输出格式**: 必须严格按照指定的JSON格式输出，根对象包含一个名为 "outlines" 的数组。
...
## Task
请严格根据“小说主线大纲”，并参考“核心简介”，为这部全新的小说创作 **前\`\${chaptersToGenerate}\`章** 的详细分镜大纲。
-   **起始章节号:** 1
