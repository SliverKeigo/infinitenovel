...
export function AiConfigManager() {
  ...
  const { 
    ...
    deleteConfig,
    useApiForEmbeddings,
    setUseApiForEmbeddings,
    embeddingModel,
    setEmbeddingModel,
    useIndependentEmbeddingConfig,
    setUseIndependentEmbeddingConfig,
    embeddingApiKey,
    setEmbeddingApiKey,
    embeddingApiBaseUrl,
  ...
  const handleToggleEmbeddingSource = (checked: boolean) => {
    setUseApiForEmbeddings(checked);
    // 重置嵌入服务，以便下次使用时重新创建
  ...
  const handleToggleIndependentConfig = (checked: boolean) => {
    setUseIndependentEmbeddingConfig(checked);
    // 重置嵌入服务，以便下次使用时重新创建
    EmbeddingPipeline.resetService();
  };
  ...
  const handleEmbeddingApiBaseUrlChange = (value: string) => {
    setEmbeddingApiBaseUrl(value || null);
    // 重置嵌入服务，以便下次使用时重新创建
    EmbeddingPipeline.resetService();
  };
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-3 flex items-center">
                <Switch 
                  ...
                  checked={currentConfig?.useApiForEmbeddings || false} 
                  onCheckedChange={(checked) => setCurrentConfig({ ...currentConfig, useApiForEmbeddings: checked })} 
                />
              </div>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input 
                ...
                onChange={(e) => setCurrentConfig({ ...currentConfig, embeddingModel: e.target.value })} 
                className="col-span-3" 
                placeholder={currentConfig?.useApiForEmbeddings ? "例如：text-embedding-ada-002" : "例如：Xenova/all-MiniLM-L6-v2"}
              />
            </div>
            {currentConfig?.useApiForEmbeddings && (
              <>
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <div className="col-span-3 flex items-center">
                    <Switch 
                      ...
                      checked={currentConfig?.useIndependentEmbeddingConfig || false} 
                      onCheckedChange={(checked) => setCurrentConfig({ ...currentConfig, useIndependentEmbeddingConfig: checked })} 
                    />
                  </div>
                </div>
                
                {currentConfig?.useIndependentEmbeddingConfig && (
                  <>
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input 
                        id="embeddingApiKey" 
                        type="password"
                        value={currentConfig?.embeddingApiKey || ''} 
                        onChange={(e) => setCurrentConfig({ ...currentConfig, embeddingApiKey: e.target.value })} 
                        className="col-span-3" 
                      />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input 
                        ...
                        value={currentConfig?.embeddingApiBaseUrl || ''} 
                        onChange={(e) => setCurrentConfig({ ...currentConfig, embeddingApiBaseUrl: e.target.value })} 
                        className="col-span-3" 
                        placeholder="可选，留空使用默认URL"
                      />
                    </div>
