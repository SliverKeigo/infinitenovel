 ...
 * @param _novelId - 小说ID (当前未使用，但保留以备将来使用)
 * @param actToPlan - 需要规划的幕布对象
 * @param currentPlotOutline - 当前小说完整的大纲
 * @param activeConfig - 激活的AI配置对象
 * @param previousActOutline - (可选) 上一幕的逐章大纲，用于提供上下文
...
`
  ...
  const plannerPrompt = `# 分幕逐章大纲规划师 v2.0
  ...
  const aiResponse = (await createAICompletion(
    {
      ...
      temperature: 0.5, // 降低随机性以获得更稳定的大纲
    },
    activeConfig,
    false // <--- 明确禁用流式调用
  )) as { choices: { message: { content: string } }[] };

  const newPlan = extractTextFromAIResponse(aiResponse);
