...
**第一幕的宏观规划（这是你必须严格遵循的整体规划）:**
${firstActInfo}

**进度分析：**
- 当前进度：第${batchStart}章到第${batchEnd}章
- 总体规划：第${actOneStart}章到第${actOneEnd}章
- 完成度：约${Math.floor(((batchEnd - actOneStart + 1) / (actOneEnd - actOneStart + 1)) * 100)}%

**重要提示：**
1. 仔细分析以上宏观规划，理解整体剧情发展脉络
2. 根据当前进度（${batchStart}-${batchEnd}章）判断应该处于哪个剧情发展阶段
3. 确保新生成的内容严格遵循宏观规划中的发展节奏
4. 注意控制剧情发展速度，不要过快或过慢推进重要情节

${plotOutline ? `**重要提示：严禁重复或相似的剧情！**
**已生成的大纲内容（这些剧情已经发生，不能重复）:**
${plotOutline}

**当前进度追踪：**
- 已完成章节数：${batchStart - actOneStart}章
- 剩余章节数：${actOneEnd - batchEnd}章
- 当前故事发展阶段：第${batchStart}章到第${batchEnd}章
- 根据宏观规划判断当前应该处于什么剧情阶段，确保按正确的节奏推进
- 请确保新增内容在剧情上有实质性推进，不得与之前的任何章节重复或相似

**请在上述大纲的基础上，继续编写第${batchStart}章到第${batchEnd}章的内容：**
- 每个新章节必须展现全新的剧情发展
- 禁止复用之前出现过的场景和事件
- 确保剧情递进，不能原地踏步
- 新增内容必须与已有大纲自然衔接，同时推动故事向前发展` : '**请从第一章开始编写大纲:**'}

**你的核心原则:**
- **放慢节奏**: 这是最高指令！你必须将宏观规划中的剧情分解成无数个微小的步骤、挑战、人物互动和支线任务。
- **填充细节**: 不要让主角轻易达成目标。为他设置障碍，让他与各种人相遇，让他探索世界。
- **禁止剧情飞跃**: 严禁在短短几章内完成一个重大的里程碑。
- **严禁重复**: 新生成的每一章都必须是全新的剧情，不得与之前的任何章节重复或相似。
