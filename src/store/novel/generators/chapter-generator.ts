...
### 冲突解决规则 (最高优先级)
- **大纲至上原则**: **本章官方大纲** 是创作的绝对蓝图和最高权威。
- **上下文冲突处理**: 如果 (相关背景资料), (文体风格指导)或任何其他上下文信息与**本章官方大纲**的核心事件、背景设定或基调发生冲突，你**必须无条件地以官方大纲为准**，并忽略所有与大纲不符的污染信息。例如，如果大纲是奇幻背景，你必须忽略所有引入的科幻元素。
...
### 冲突解决规则 (最高优先级)
- **大纲至上原则**: **本章必须实现的关键事件** 和 **本章官方大纲** 是创作的绝对蓝图和最高权威。
- **上下文冲突处理**: 如果 (相关背景资料), (文体风格指导), (前章衔接信息) 或任何其他上下文与大纲的核心事件、背景设定或基调发生冲突，你**必须无条件地以大纲为准**，并忽略所有与大纲不符的污染信息。你的任务是基于大纲进行创作，而不是试图整合所有可能矛盾的上下文。
